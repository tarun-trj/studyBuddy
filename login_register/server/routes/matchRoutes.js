...
function handleData() {
    EmployeeModel.find().select('email branch')
        ...
        .then(joinedData => {
            ...
            const maxMatching = greedyMaximumWeightMatching(adjacencyList);
            console.log("Maximum Weight Matching:", maxMatching);

            displayMatching(maxMatching, emails);
        })
        .catch(error => {
            console.error('Error during data handling operation:', error);
        });
...
function displayMatching(matching, emails) {
    ...
    matching.forEach(({ email1, email2 }) => {
        matchMap.set(email1, email2);
        matchMap.set(email2, email1);
    });
